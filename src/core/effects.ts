...
export function releasingEffects(allowed: Set<string>, send: SendFn = defaultSend): {
  effects: Effects; released: string[]; dropped: string[]; flush(): string[];
} {
  const released: string[] = [], dropped: string[] = [];
  const pending: { id: string; kind: string; payload: Record<string, unknown> }[] = [];
  const seen = new Map<string, number>();
  return {
    released, dropped,
    flush(): string[] {
      ...
      try {
        for (const p of pending) { send(p.kind, p.payload); sentIds.push(p.id); }
      } catch {
        // A transport failure stops the flush where it stopped. Everything that already
        // went out stays reported — state and transport cannot be atomic, so the honest
        // answer is what actually left, not a pretence that nothing did.
      } finally {
        pending.length = 0;
      }
      return sentIds;
    ...
    effects: {
      async notify(to, message) {
        ...
        if (allowed.has(id)) { released.push(id); pending.push({ id, kind: 'notify', payload }); }
